...
#   - _llvmup             : Native Zsh completion script for llvmup commands.
#
...
#   LLVMUP_ZSH_COMPLETION_DIR: Directory for zsh completion (default: $LLVMUP_PREFIX/share/zsh/site-functions)
#
# Examples:
...
EOF
    ...
    if [ -f "$HOME/.zshrc" ]; then
        if add_to_profile "$HOME/.zshrc"; then
            zsh_functions_configured=true
        fi

        if grep -q "LLVMUP Zsh Completion" "$HOME/.zshrc"; then
            print_info "LLVMUP zsh completion already configured in $HOME/.zshrc"
        else
            cat >> "$HOME/.zshrc" << EOF

# LLVMUP Zsh Completion - Start
# Auto-generated by LLVMUP installer - safe to remove
...
# LLVMUP Zsh Completion - End
EOF
            print_success "Added LLVMUP zsh completion to $HOME/.zshrc"
            zsh_completion_configured=true
        fi
    else
        print_info "Creating $HOME/.zshrc..."
        touch "$HOME/.zshrc"
        if add_to_profile "$HOME/.zshrc"; then
            zsh_functions_configured=true
        fi
...
# LLVMUP Zsh Completion - End
EOF
        print_success "Created $HOME/.zshrc with LLVMUP functions and zsh completion"
        zsh_completion_configured=true
    fi
else
    ...
    print_warning "For zsh, add '$ZSH_COMPLETION_DIR' to fpath and run compinit"
    profile_configured=false
    zsh_functions_configured=false
    zsh_completion_configured=false
fi
...
echo "  llvmup activate     : Activate an installed LLVM version"
echo "  llvmup deactivate   : Deactivate the current LLVM version"
echo "  llvmup vscode-activate : Configure VSCode for an installed version"
echo "  llvmup status       : Show current LLVM status"
echo "  llvmup list         : List installed LLVM versions"
echo "  llvmup help         : Show the full LLVMUP help guide"
echo ""
...
echo "  llvm-config-apply   : Install from project configuration"
echo "  llvm-config-activate: Activate from project configuration"
echo "  llvm-help           : Show the detailed LLVMUP help guide"
echo ""

print_info "Completion assets:"
...
if [ "$REQUIRES_SUDO" = false ]; then
    ...
    if [ "$zsh_functions_configured" = true ]; then
        print_success "LLVMUP functions have been added to your ~/.zshrc."
    fi
    if [ "$zsh_completion_configured" = true ]; then
        print_success "LLVMUP zsh completion has been added to your ~/.zshrc."
    fi
    if [ "$zsh_functions_configured" = false ] && [ "$zsh_completion_configured" = false ]; then
        print_info "To manually enable zsh, source \"$FUNCTIONS_FILE\", add '$ZSH_COMPLETION_DIR' to fpath, and run compinit."
    fi
else
    ...
    print_info "For zsh, users should add '$ZSH_COMPLETION_DIR' to fpath and run compinit."
fi
